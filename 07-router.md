...
3. **rewrite_level ≥ 2일 때**: 첫 segment를 prefix로 보고:
   ...
   - 등록된 mid의 내부 URL이 비어 있으면 인덱스 액션(`default_index_act`)을 선택.
   - 내부 URL이 있으면 해당 모듈의 라우트를 순회해 매칭되는 액션 선택.
   - 매칭되지 않은 non-empty URL은 인덱스 액션으로 폴백하지 않고 forwarded 라우트와 generic `mid/act` 패턴 등을 계속 탐색.
4. **그렇지 않으면**: forwarded 라우트와 글로벌 라우트 매칭 시도.
5. 최종 결과는 `Rhymix\Framework\Request` 객체에 채워져 반환.

`Request` 객체는 `module`, `mid`, `act`, 그 외 임의 변수, `getRouteStatus()`(200/404 등), `args` 배열을 제공한다. 또한 `cache_control`, `check_csrf`, `is_forwarded`, `is_indexable`, `enable_session` route option을 가진다 (`Request.php:72-79`). 액션이 매칭되면 `Router::_fillActionProperties()`가 module.xml 메타데이터를 이 option들로 옮긴다 (`Router.php:495-517`).
