...
각 도구는 본체 루트에 놓인 규칙 파일을 자동으로 읽는다. 그 파일 안에 **"코드 탐색 전에 `docs/llms.txt`를 먼저 읽어라"** 한 줄만 명시하면 어떤 어시스턴트든 이 문서를 컨텍스트로 활용한다.

| 도구 | 본체 루트에 둘 파일 | 비고 |
|---|---|---|
| Claude Code | `CLAUDE.md` | Rhymix 본체에 이미 포함되어 있음. 추가 설정 불필요 |
