...
댓글 모듈이 사용하는 숫자형 상태 호환 상수. 현재 문서 상태는 `PUBLIC` 같은 문자열을 사용하고, 파일 테이블에는 이 상태 필드가 없다.
...
| 함수 | 비고 |
|---|---|
| `is_html_content($str)` | 줄 경계의 `p`/`div`/`h1`~`h6`, 다수의 `br`, style이 있는 `span`을 세어 HTML처럼 보이는지 판정하는 heuristic |
...
| `getTimeGap($date, $format='Y.m.d')` | 24시간 미만은 상대 시간(60초 미만도 1분 전으로 보정), 그 이후는 지정 포맷 |
