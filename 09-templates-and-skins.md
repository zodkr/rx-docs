...
여러 줄 서버측 주석은 v1 전용이다. v2 엔진은 여는 표식과 닫는 표식이 같은 줄에 있는 한 줄 주석만 제거하므로, `.blade.php`로 마이그레이션하거나 두 엔진을 함께 쓰는 템플릿에서는 여러 줄 주석을 사용하지 않는다 (아래 v2의 "주석과 `@verbatim`의 실제 범위" 참고).
...
| 3 | `_removeComments` | 한 줄 `<!--// ... -->`, `{{-- ... --}}` 제거. 여러 줄 주석은 제거되지 않음 |
