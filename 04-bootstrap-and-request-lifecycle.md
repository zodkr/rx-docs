...
| `RX_POST` | POST 메서드인가 |
| `RX_WINDOWS` | 윈도우 OS인가 |
| `RX_STATUS_*` | 콘텐츠 상태 코드 11종 (`PUBLIC`, `SECRET`, `TEMP`, `TRASH`, …) |
...
4. **HTTP 요청 분석** — `setRequestMethod()`, `Router::parseURL()`로 URL을 모듈/액션/변수로 분해, `setRequestArguments`(`:207-219`).
...
9. **세션 핸들러 등록** (DB 세션일 때) → `Session::start(false)` (`:329-358`).
10. **출력 버퍼 시작** — `ob_start()` (HTTP일 때만) (`:362-365`).
11. **인증 정보 설정** — 로그인 회원이면 `MemberController::setSessionInfo()`, 아니면 `is_logged=false` (`:367-379`).
12. **Debug 활성 여부 결정** — `Debug::isEnabledForCurrentUser()` (`:382`).
13. **`current_url`/`request_uri` 설정** — IDN xn-- 디코딩 포함 (`:384-402`).
14. **Mobile 컨텍스트 변수** — `m=1` 또는 `0` (`:405`).
15. **사이트 잠금 검사** — `config('lock.locked')`면 잠금 페이지 강제 (`:408-411`).
...
1. **미등록 도메인 처리** (`:127-166`):
   ...
   - `config('url.unregistered_domain_action')` 값으로 분기:
     - `redirect_301` (기본) / `redirect_302` — 기본 도메인으로 리다이렉트.
     - `block` — 404 출력.
     - `display` — 임시 도메인으로 처리 계속.
2. **`success_return_url`/`error_return_url`** 내부 URL 검증 (`:170-177`). 외부 URL이면 제거.
3. **라우터 에러 처리** — `route->getRouteStatus() > 200`이면 404 (`:180-186`).
4. **entry → document_srl 변환** (`:189-196`).
5. **document_srl로 module_info 역추적** (`:202-209`).
6. **mid → module_info 매핑** (`:212-215`).
7. **도메인 일치 검증** — 다른 도메인의 mid면 404 (`:218-226`).
8. **기본 모듈 결정** (`:229-232`).
9. **index 문서 설정** (`:235-238`).
10. **레이아웃 srl 결정** — PC/Mobile 분기, `-1`(사이트 기본)/`-2`(PC와 공유) 처리 (`:259-285`).
11. **컬러 스키마 설정** (`:293`).
12. **트리거 호출** — `moduleHandler.init.after` (`:314`). false 반환 시 에러로 진행.

## 5. `procModule()`

`ModuleHandler::procModule()` (`:331`).

1. **에러 처리** — `$this->error`가 있으면 message 모듈로 변환해 반환 (`:337-340`).
2. **module.xml 로드** — `ModuleModel::getModuleActionXml($this->module)` (`:343`).
3. **`act` 결정** — 미지정 시 `default_index_act` 폴백. 그래도 없으면 404 (`:346-364`).
4. **타입/kind/meta-noindex 결정** — admin이 포함된 액션은 `kind='admin'` (`:367-381`).
5. **HTTP 메서드 검사** — `action.method`에 현재 메서드가 없으면 405 (`:384-391`).
6. **CSRF 검증** — 비-GET/HEAD/OPTIONS + `check_csrf !== 'false'` + 설치됨이면 `Security::checkCSRF()`. 실패 시 403 (`:394-400`).
7. **standalone 검사** — `standalone === 'false'`이고 mid 없으면 403 (`:403-413`).
8. **`use_mobile` 조정** — 모듈이 모바일 미지원이면 Mobile::setMobile(false) (`:415-423`).
9. **회원 메뉴 lang 재할당** (`:426-430`).
10. **모듈 인스턴스 생성** (`:433-474`):
...
11. **액션 미존재 시 forward 탐색** — 액션 이름에서 모듈명 역추론 또는 `getActionForward()` (`:482-628`).
12. **ruleset 검증** — `ruleset=` 지정 시 `Validator` 실행. v2 모듈에서는 deprecation warning (`:630-676`).
13. **`setAct` + `setModuleInfo($module_info, $xml_info)`** — 스킨/레이아웃/권한 설정 (`:678-681`).
14. **HTML 응답이면 도메인 헤더/푸터/타이틀 주입** (`:683-701`).
15. **`$oModule->proc()` 호출** (`:711`).
16. **HTML 비동기 응답이 아니면** 에러/메시지/redirect 결과를 세션 INPUT_ERROR로 저장 (`:713-751`).
...
`ModuleHandler::displayContent()` (`:1004`).

1. 모듈 객체 유효성/DB 연결 검사.
2. **트리거** — `moduleHandler.proc.after` (`:1023`).
3. HTML 응답이면:
   - `getRedirectUrl()` 있으면 즉시 리다이렉트 (`:1034-1058`).
   - 에러 있으면 `MessageView`로 변환 (`:1061-1086`).
   - 레이아웃 srl 확정 (`:1089-1109`).
   - 레이아웃 정보 로드 → `extra_vars`/`menus`/`menu_count`/`layout_info`를 컨텍스트에 주입 (`:1111-1195`).
4. HTTP 상태 코드 결정 (`:1213-1219`).
5. `DisplayHandler::printContent($oModule)` 호출 (`:1222-1223`).
...
각 핸들러의 `toDoc()`이 본문 문자열을 만들고, `printContent()`가 HTTP 헤더와 함께 출력한다. HTMLDisplayHandler는 `layout.before` → `display.before` → addon `before_display_content` → echo → `display.after` 순으로 트리거를 발사한다.
...
| `HTMLDisplayHandler` 레이아웃 합성 직전 | `layout` | `before` |
