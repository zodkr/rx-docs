...
- **권한** — `module_grants`의 `name`(grant ID)과 숫자형 `group_srl`로 저장한다. 양수는 실제 회원 그룹이며, `0`은 누구나, `-1`/`-2`는 회원, `-3`은 매니저, `-4`는 비로그인을 뜻한다 (`modules/module/models/Permission.php:45-84`).

회원 정보·비밀번호 변경 액션은 성공 후 캐시를 비우고 세션 갱신을 예약한다. 실제 갱신 시점은 [세션과 인증](../15-session-and-auth.md#회원-정보비밀번호-변경-후-갱신) 참고. 로그인 폼의 복귀 URL은 `MemberView::checkRefererUrl()`이 검증하고 이스케이프한 값을 사용한다 ([보안](../19-security.md#로그인-후-돌아갈-url)).
