...
9. **세션 핸들러 등록** (DB 세션일 때) → `Session::start(false)` (`:329-355`).
...
14. **HTML 응답이면 도메인 헤더/푸터/타이틀 주입** (`:672-690`).
15. **`$oModule->proc()` 호출** (`:700`).
16. **HTML 비동기 응답이 아니면** 에러/메시지/redirect 결과를 세션 INPUT_ERROR로 저장 (`:702-740`).
