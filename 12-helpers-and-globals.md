...
| 상수 | 의미 | 정의 위치 |
|---|---|---|
| `RX_VERSION` | 현재 Rhymix 버전 (예: `'2.1.33'`) | `:6` |
...
| `escape_sqstr($str)` | SQL 작은따옴표 |
| `escape_dqstr($str)` | SQL 큰따옴표 |
| `explode_with_escape($delim, $str, $escape='\\')` | 이스케이프 인식 split |
...
| `countobj($var)` | array/Countable이면 count, 아니면 0 |
...
| 함수 | 비고 |
|---|---|
| `include_in_clean_scope($file, $vars=[])` | 격리된 스코프로 include |
...
autoloader 분기 2가 사용하는 정적 매핑 (lc 클래스명 → 파일 경로). 약 35개 항목.
...
| `setUserSequence($seq, $member_srl)` | 사용자에 시퀀스 매핑 |
...
| `getDestroyXeVars($vars)` | XE 시스템 변수 제거 (act/mid/vid 등) |
| `getNumberingPath($no, $size=3)` | `12345` → `'12/34/5/'` (해시 디렉토리) |
