...
선택은 `config('cache.type')` (배열 가능 — 다중 폴백).

```php
'cache' => [
    'type' => ['redis', 'file'],         // redis 실패 시 file로
    'ttl' => 86400,
...
- 키 조회 시 `{prefix}_{group}_{version}_{key}` 형식 — 옛 버전 키는 자연 만료.
- 대량 키 일괄 무효화에 효율적.

### prefix

`config('cache.prefix')`는 멀티 사이트/멀티 인스턴스에서 키 충돌 방지. 자동으로 도메인/사이트 정보가 부착되기도 함.

### TTL

- 기본 `config('cache.ttl')` (보통 86400 = 1일).
- `Cache::set($key, $value, 0)` — 만료 없음 (드라이버 지원 시).
...
`config('cache.truncate_method')`:

- `'delete'` (기본) — 개별 키 삭제 (드라이버에 따라 비쌀 수 있음).
- `'empty'` — DB cache의 경우 `TRUNCATE TABLE`.
...
| 드라이버 | 비고 |
|---|---|
| `db` | RDBMS (`queue` 테이블). 기본 |
| `redis` | Redis ZSet 기반 (정확한 timestamp 정렬) |
| `dummy` | 즉시 무시 |
...
`Queue.php:110-116`의 docblock에 정의된 4개 형식만 지원된다.
...
CLI가 아닐 때 cron.php는 자체적으로 `autoload.php`를 require하고 `Context::init()`을 호출한 뒤 `Queue::checkKey($_GET['key'])`로 인증한다. 키 불일치 시 403 응답.
...
```php
$ok = Rhymix\Framework\Queue::checkKey($_GET['key'] ?? '');
```
...
- 활성 작업: `queue` 테이블의 행.
- 실패: `attempts` 카운트가 증가. 임계 초과 시 dead letter로 분리.
- 완료: 즉시 삭제.
