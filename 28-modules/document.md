...
$oDoc->getStatus();             // 'PUBLIC'/'SECRET'/... 문자열
...
| `document_trash` | 레거시 휴지통 메타 테이블. 현재 `moveDocumentToTrash()`는 통합 `trash` 테이블에 직렬화한 뒤 원본 `documents` 행을 삭제하며, `document.insertTrash`의 현행 호출자는 없음. |
...
| `module.procModuleAdminCopyModule` (after) | `triggerCopyModule` (controller) | 모듈 복사 시 document 모듈별 설정 복사 |
| `file.deleteFile` (after) | `triggerAfterDeleteFile` (controller) | 첨부 삭제 후 문서의 `uploaded_count` 재계산 |
...
문서 테이블은 숫자형 `RX_STATUS_*` 상수가 아니라 다음 문자열을 `status` varchar 컬럼에 저장한다 (`modules/document/schemas/documents.xml:34`, `modules/document/document.class.php:29-40`). 숫자형 상수는 comment 모듈 호환용이다.
...
| `TRASH` | 휴지통 |
| `CENSORED` | 블라인드 |
| `CENSORED_BY_ADMIN` | 관리자 블라인드 |
| `DELETED` | 삭제 |
| `DELETED_BY_ADMIN` | 관리자 삭제 |
| `OTHER` | 기타 |

코드에서 설정값을 얻을 때는 `DocumentModel::getConfigStatus($key)`를 사용한다(`$key` 예: `public`, `secret`, `temp`). DB/API 값은 문자열이며 숫자형 `RX_STATUS_*`와 혼동하지 않는다.
