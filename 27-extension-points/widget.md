...
$html = $oWidgetController->execute($widget_name, $args);  // 첫 인자는 위젯 이름, 둘째 인자는 args 객체
```
...
위젯 마크업의 `widget_cache` 속성(예: `widget_cache="1m"`)을 지정하면 `WidgetController::getCache()`가 위젯별 캐싱을 수행한다. 자주 변하지 않는 위젯은 캐싱 권장.

```php
public function proc($args)
{
    $cache_key = 'hello:' . md5(serialize($args));
    if ($cached = Rhymix\Framework\Cache::get($cache_key)) {
        return $cached;
    }
    // ... 처리 ...
    $html = TemplateHandler::getInstance()->compile($tpl_path, '1');
    Rhymix\Framework\Cache::set($cache_key, $html, 600);
    return $html;
}
```
